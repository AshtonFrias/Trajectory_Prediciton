...
parser.add_argument('--loss_mode', type=int, default=2,
                    help='Set loss mode: 0=MinADE, 1=MinFDE, 2=MinADE+FDE, 3=Other')
...
class MinFDE_loss(nn.Module):
    def __init__(self):
        super(MinFDE_loss, self).__init__()   

    def __call__(sef, traj, traj_gt):
        """
        Computes average displacement error for the final trajectory in a set, with respect to ground truth
        ...
        :return err:  average minFDE over batch members
        """
        batch_size, num_modes = traj.shape[:2]
        seq_len = traj_gt.shape[1]

        # Reshape predictions to [batch, num_modes, seq_len, 2]
        traj = traj.view(batch_size, num_modes, seq_len, 2)

        # Get final position of prediction and ground truth 
        final_pred = traj[:, :, -1, :]               # [batch, num_modes, 2]
        final_gt = traj_gt[:, -1, :]                 # [batch, 2]

        # Compute Euclidean distance pred and ground truth
        err = final_pred - final_gt[:, None, :]
        square_err = err ** 2
        sum_squared = torch.sum(square_err, dim=-1)
        fde_error = torch.sqrt(sum_squared)

        # Take the best minimum error in the batch
        min_fde = fde_error.min(dim=1).values 
...
class MinADE_loss(nn.Module):
  ...
  def __call__(self, traj, traj_gt):
    ...
    # Calculate the Ecliden distance between the ground truth and prediticted trajectory
    square_err = err ** 2                    # square the difference between pred and gt
    ts_err = torch.sum(square_err, dim=3)     # add over sequence length
    square_root = torch.sqrt(ts_err)          # take the square root

    # Next, we find the average displacement error over the proposed trajectories
    avg_displacement_error = torch.mean(square_root, dim=-1) 

    # Next, find the minumum displacement error of all the proposed trajectories
    min_avgerage, _ = torch.min(avg_displacement_error, dim=1) 

    # Lastly, average it over the batch memebers to get the loss value
    err = torch.mean(min_avgerage) 
    return err
  
class Combined_Loss(nn.Module):
    def __init__(self, weight_ade=1.0, weight_fde=1.0):
        super(Combined_Loss, self).__init__()
        self.weight_ade = weight_ade
        self.weight_fde = weight_fde
        self.ade_loss = MinADE_loss()
        self.fde_loss = MinFDE_loss()

    def __call__(self, traj, traj_gt):
        ade = self.ade_loss(traj, traj_gt)
        fde = self.fde_loss(traj, traj_gt)
        loss = self.weight_ade * ade + self.weight_fde * fde
        return loss
...
def train(epoch, model, reg_criterion, cls_criterion, optimizer, train_dataloader, motion_modes,just_x_y,num_k,ped_num_k, loss_mode):
    model.train()
    total_loss = []
    ...
    for i, (ped_traj,nei_traj,mask_nearest) in enumerate(train_dataloader):
        ...
        optimizer.zero_grad()
        pred_traj, scores = model(ped_obs.cuda(), neis_obs.cuda(), motion_modes.cuda(),mask_nearest.cuda(), closest_mode_indices.cuda(), num_k=num_k, ped_num_k =ped_num_k,loss_mode=loss_mode)
        # print('pred_traj.shape',pred_traj.shape)
        # print('gt.shape',gt.shape)
        if args.loss_mode == 0: # MinADE
            reg_loss = reg_criterion(pred_traj, gt.cuda())
        elif args.loss_mode == 1: # MinFDE
            reg_loss = reg_criterion(pred_traj, gt.cuda())
        elif args.loss_mode == 2: # MinADE+FDE
            reg_loss = reg_criterion(pred_traj, gt.cuda())
        else:
...
def test(model, test_dataloader, motion_modes, just_x_y,num_k,ped_num_k):
    ...
    for i, (ped_traj,nei_traj,mask_nearest) in enumerate(test_dataloader):
        ...
        with torch.no_grad():
            ...
            pred_trajs, scores = model(ped_obs.cuda(), neis_obs.cuda(), motion_modes.cuda(), mask_nearest.cuda(), None, test=True, num_k= num_k, ped_num_k=ped_num_k,loss_mode=1)
...
if __name__ == '__main__':
    ...
    if args.loss_mode == 0: # MinADE
        ...
        print("minFDEloss")
        reg_criterion = MinFDE_loss().cuda()
    elif args.loss_mode == 2: # MinADE+FDE
        print("MinADE+FDE_(0.9,0.5)")
        reg_criterion = Combined_Loss(weight_ade=1.25, weight_fde=1.25).cuda()
    else:
    ...
    for ep in range(args.epoch):    
        ...
        total_loss = train(ep, model, reg_criterion, cls_criterion, optimizer, train_loader, motion_modes,args.just_x_y, args.num_k, args.ped_num_k,args.loss_mode)
        
        ade, fde, num_traj, ade_vector, fde_vector = test(model, test_loader, motion_modes, args.just_x_y, args.num_k, args.ped_num_k)
        if args.lr_scaling:
            scheduler.step()    

        if not os.path.exists(args.checkpoint +'_'+ args.dataset_name +'_n_clusters_' + str(args.n_clusters)  + '_num_K_' +str(args.num_k) + '_minADE_Loss_' + str(args.loss_mode)):
            os.makedirs(args.checkpoint +'_'+ args.dataset_name +'_n_clusters_' + str(args.n_clusters)  + '_num_K_' +str(args.num_k) + '_minADE_Loss_' + str(args.loss_mode))
        fde_ade_file = args.checkpoint +'_'+ args.dataset_name +'_n_clusters_' + str(args.n_clusters)  + '_num_K_' +str(args.num_k) + '_minADE_Loss_' + str(args.loss_mode) +'/' + 'fde_ade'
        if min_fde + min_ade > ade + fde:
            ...
            torch.save(model.state_dict(), args.checkpoint +'_'+ args.dataset_name +'_n_clusters_' + str(args.n_clusters)  + '_num_K_' +str(args.num_k) + '_minADE_Loss_' + str(args.loss_mode) + '/best.pth')  # OK
